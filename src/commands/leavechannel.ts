...
export default async function leavechannel(
  ...
  interaction.reply(
    `User <@${interaction.user.id}> has left this channel (<#${interaction.channelId}>)`
  )
}
...
export const leavechannelBuilder = new SlashCommandBuilder()
  ...
  .setDescription(
    'Leave this Discord Channel. (Note you must ask someone else in the channel to add you back.)'
  )
