...
export default async function invitechannel(_client: Client, interaction: CommandInteraction) {
  ...
  interaction.reply(
    `Invited ${memberCount} member${memberCount === 1 ? '' : 's'} of <#${channel.id}> to <#${interaction.channel.id}>`
  )
}
